...
module.exports = async (ctx) => {
    ...
    const items = list
        ...
        .map((i) => {
            ...
            const topicPath = $item.find('h3.title a').attr('href');
            ...
            const link = topicPath ? new URL(topicPath, url).href : undefined;
            return link ? { title, link, guid: link, author, pubDate } : undefined;
        })
        .filter((item) => item !== undefined);
    const out = [];
    for await (const result of asyncPool(2, items, async (item) => {
        const detailUrl = item.link.split('#')[0];
        const res = await got({
            method: 'get',
            url: detailUrl,
            headers: {
            ...
            .map((el) => {
                ...
                $comment.find('[href], [src]').each((index, element) => {
                    ...
                    for (const attribute of ['href', 'src']) {
                        ...
                        if (value) {
                            $element.attr(attribute, new URL(value, detailUrl).href);
                        }
                    }
                });
