...
module.exports = async (ctx) => {
    ...
    logger.info(`Naixi RSS: PUPPETEER_WS_ENDPOINT = ${config.puppeteerWSEndpoint ? 'configured' : 'not configured'}`);
    ...
    if (config.puppeteerWSEndpoint) {
        ...
        try {
            ...
            page.on('response', async (res) => {
                if (res.url().includes('forum.php?mod=rss')) {
                    try {
            ...
            if (xmlContent && xmlContent.includes('<?xml')) {
                ...
                response = bodyText && bodyText.includes('<?xml') ? bodyText.substring(bodyText.indexOf('<?xml')) : bodyText;
            }

            logger.info(`Naixi RSS: Puppeteer intercepted xmlContent (first 500 chars): ${xmlContent?.substring(0, 500)}`);
